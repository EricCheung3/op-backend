...
public class ParsedReceiptImpl implements ParsedReceipt{
    ...
    private ParsedReceiptImpl(final StoreChain chain, final List<Item> items, final  Map<ReceiptFieldType, ValueLine> map){
        this.chain=chain;
        this.items=items;
        this.fieldToValueMap=map;
    }

    private ParsedReceiptImpl(final StoreChain chain, final List<Item> items, final  Map<ReceiptFieldType, ValueLine> map, final StoreBranch branch){
        this(chain, items, map);
        this.branch=branch;
    }

    public static ParsedReceiptImpl fromChainItemsMapBranch(final StoreChain chain, final List<Item> items, final  Map<ReceiptFieldType, ValueLine> map, final StoreBranch branch){
